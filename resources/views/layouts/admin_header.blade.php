...
<style>
    ...
    /* FROSTED HOVER FOR DARK BACKGROUND */
    ...
    /* INVERTED AVATAR FOR VISIBILITY */
...
<div class="top-header no-print">
    ...
    <div class="header-actions">
        ...
        <div class="dropdown">
            <div class="user-profile" data-bs-toggle="dropdown" aria-expanded="false" title="{{ $user->firstname ?? 'Admin' }} {{ $user->lastname ?? '' }}">
                @if($user && $user->image)
            ...
            <ul class="dropdown-menu dropdown-menu-end shadow border-0 mt-2">
                <li><h6 class="dropdown-header fw-bold text-dark">{{ $user->firstname ?? 'Admin' }} {{ $user->lastname ?? '' }}</h6></li>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header" style="background: linear-gradient(135deg, #101954 0%, #0a4d9c 100%); color: white;">
                <h5 class="modal-title fw-bold"><i class="fas fa-user-circle me-2"></i>Admin Profile</h5>
            ...
            <form id="profileUpdateForm" action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body bg-light p-4">
                    ...
                    <div id="avatarDisplayArea" class="d-flex align-items-center mb-4 pb-4 border-bottom">
                        ...
                        <div>
                            ...
                            <p class="text-muted mb-0"><span class="badge bg-dark bg-opacity-10 text-dark border border-dark px-3 py-1">{{ $user->role ?? 'Admin' }}</span></p>
                        </div>
                    </div>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6 mt-3">
                            ...
                            <input type="text" class="form-control text-muted" value="{{ $user->department ?? 'Administrator' }}" disabled>
                        </div>
                        <div class="col-md-6 mt-3">
                ...
                <div class="modal-footer border-0 bg-white">
                    ...
                    <button type="submit" class="btn btn-primary px-4 fw-bold shadow-sm"><i class="fas fa-save me-2"></i>Save Changes</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        function fetchSearchResults(query) {
            ...
            fetch(`{{ url('/admin/global-search') }}?q=${encodeURIComponent(query)}`, { headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' } })
        ...
        function fetchNotifications() {
            // ADMIN SPECIFIC ROUTE
            fetch(`{{ url('/admin/notifications/fetch') }}`, { 

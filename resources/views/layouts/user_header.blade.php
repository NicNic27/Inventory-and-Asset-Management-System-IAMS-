...
<div class="top-header no-print">
    ...
    <div class="header-actions">
        ...
        <div class="global-search-container" id="searchContainer">
            <input type="text" id="globalSearchInput" placeholder="Search my requests..." autocomplete="off">
        ...
        <div class="dropdown">
            <div class="user-profile" data-bs-toggle="dropdown" aria-expanded="false" title="{{ $user->firstname ?? 'User' }} {{ $user->lastname ?? '' }}">
                @if($user && $user->image)
...
<div class="modal fade no-print" id="profileEditModal" tabindex="-1" aria-labelledby="profileEditModalLabel" aria-hidden="true" style="z-index: 1060;">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content border-0 shadow-lg">
            ...
            <form id="profileUpdateForm" action="{{ url('/user/profile') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="modal-body bg-light p-4">
                    ...
                    <div id="avatarDisplayArea" class="d-flex align-items-center mb-4 pb-4 border-bottom">
                        ...
                        <div>
                            ...
                            <p class="text-muted mb-0"><span class="badge bg-primary bg-opacity-10 text-primary border border-primary px-3 py-1">Division User</span></p>
                        </div>
                    </div>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6 mt-3">
                            <label class="form-label fw-bold text-muted small text-uppercase">Division / Office</label>
                            <input type="text" class="form-control text-muted" value="{{ $user->department ?? 'Not Assigned' }}" disabled>
                            <small class="text-muted" style="font-size: 0.75rem;">Contact administrator to change division.</small>
                        </div>
                        <div class="col-md-6 mt-3">
...
<script>
    // --- GLOBAL IMAGE CROPPER & FILTER LOGIC ---
    let cropper = null;
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        function renderNotifications() {
            ...
            let activeNotifs = currentNotifData.filter(n => !dismissed.includes(n.id.toString()));
            let unreadCount = activeNotifs.filter(n => !readNotifs.includes(n.id.toString())).length;
            ...
            if (activeNotifs.length === 0) {
                html = '<div class="p-5 text-center text-muted"><i class="fas fa-check-circle fs-1 text-success mb-3 d-block"></i>No new notifications</div>';
            } else {
                activeNotifs.forEach(notif => {
                    let closeBtn = `<i class="fas fa-times close-single-notif" data-id="${notif.id}" title="Clear"></i>`;
                    let unreadClass = !readNotifs.includes(notif.id.toString()) ? 'unread-notif' : '';
                    html += `
                    <div class="notif-item ${unreadClass}">
                        <a href="${notif.url}" class="notif-link d-flex align-items-start p-3 text-decoration-none text-dark pe-5" data-id="${notif.id}">
        ...
        function fetchNotifications() {
            ...
            .then(data => { 
                if (data && data.notifications) { 
                    currentNotifData = data.notifications; 
                    renderNotifications(); 
                } 
            })
            .catch(err => {
        ...
        function openDrawer() { notifDrawer.classList.add('open'); notifBackdrop.classList.add('show'); }

...
<head>
    ...
    <style>
        ...
        /* TABLE-BASED SIGNATURE BLOCK */
        .po-paper .sig-container-table { border: 1px solid black; border-top: none; }
        .po-paper .sig-container-table td { border: none !important; padding: 10px !important; }
        .po-paper .sig-line-container { border-bottom: 1px solid black; width: 85%; margin: 35px auto 2px auto; font-weight: bold; text-transform: uppercase; text-align: center; }
...
<body>
    ...
    <div class="main-content no-print">
        ...
        <div class="table-card">
            <form action="{{ url('/admin/po') }}" method="GET" id="filterForm" class="d-flex justify-content-between align-items-center mb-3">
                <div class="input-group" style="width: 300px;">
                    <span class="input-group-text bg-white border-end-0"><i class="fas fa-search text-muted"></i></span>
                    <input type="text" name="search" id="poSearchInput" class="form-control border-start-0 ps-0" placeholder="Search PO..." value="{{ request('search') }}">
                </div>
                ...
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-end p-3 shadow-lg border-0" style="width: 250px;">
                        ...
                        <a href="{{ url('/admin/po') }}" class="btn btn-sm btn-light w-100 border text-danger fw-bold">Clear Filters</a>
                    </div>
                </div>
            </form>
                        ...
                        <tr class="clickable-row" data-id="{{ $po->id }}">
                            ...
                            <td class="text-center">
                                <div class="d-flex justify-content-center gap-1">
                                    <button class="btn btn-sm btn-light border text-primary" title="View & Print" onclick="viewPO({{ $po->id }})">
                    ...
                    <div class="po-paper">
                        ...
                        <table>
                            <thead style="background-color: #e5e7eb; text-align: center; font-weight: bold;">
                                <tr>
                                    <td style="width: 10%;">Stocks No.</td>
                        ...
                        <table class="sig-container-table">
                            <tr>
                                <td style="width: 50%; text-align: center;">
                                    <div style="text-align: left;">Conforme:</div><br><br>
                                    <div class="sig-line-container">&nbsp;</div>
                                    <div style="font-size: 10px;">Signature over Printed Name of Supplier</div>
                                    <div style="margin-top: 10px;">DATE: ___________</div>
                                </td>
                                <td style="width: 50%;" class="rd-left-box">
                                    <div>Very truly yours,</div><br><br>
                                    <div id="v-auth-name" class="sig-line-container"></div>
                        ...
                        <table>
                            <tr>
                                <td style="width: 55%;">Fund Cluster: _________________<br>Funds Available: _______________<br><br><div id="v-acc-name" style="border-bottom: 1px solid black; width: 80%; margin: 0 auto; text-align: center; font-weight: bold; text-transform: uppercase;"></div><div id="v-acc-designation" style="text-align: center; font-weight: bold; font-size: 10px;">ACCOUNTANT II</div><div style="text-align: center; font-size: 9px;">Signature over Printed Name of Chief Accountant/Head of Accounting Division/Unit</div></td>
    ...
    <script>
        ...
        function viewPO(id) {
            fetch(`/admin/po/${id}`)
            .then(res => {
                if(!res.ok) throw new Error('Failed to load PO data.');
                return res.json();
            })
            .then(data => {
                currentPoData = data;
                ...
                for(let i=0; i < (8 - itemsArray.length); i++) {
                    vBody.innerHTML += `<tr class="empty-row"><td></td><td></td><td></td><td></td><td></td><td></td></tr>`;
                }
            ...
            .catch(err => {
                console.error(err);
                Swal.fire('Error', 'Failed to load PO data. Make sure migrations are updated.', 'error');
            });
        }

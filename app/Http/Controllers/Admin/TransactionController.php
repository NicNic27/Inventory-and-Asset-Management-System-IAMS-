...
class TransactionController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Crucial: Increase the group_concat max length to prevent truncation 
        ...
        $transactions = DB::table('transactions as t')
            ...
            // Join the RIS table to grab the requester's name based on the remarks
            ->leftJoin('ris_requests as r', function($join) {
                $join->whereRaw("t.remarks LIKE CONCAT('%', r.ris_no, '%')");
            })
            ->select(
                // Group by the RIS Remarks if it's an OUT transaction, otherwise group by ID
                ...
                // Keep the ID for the delete action
                DB::raw("MAX(t.id) as id")
            )
            ->groupBy('group_key')
